...
name of the drawing group, the last parameter is always the color.
 A list of named colors can be found in `vizkit3d_debug_drawings/DebugDrawingColors.hpp`. If none of the named colors suits you, you can always define your own. A color is just a `Eigen::Vector4d` containing RGBA values.
...
{
    V3DD::CONFIGURE_DEBUG_DRAWINGS_USE_PORT(this, {"some_drawing"});
    //your code here
    V3DD::FLUSH_DRAWINGS();
    }
```
